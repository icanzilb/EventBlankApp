...
extension NSDate {
    ...
    // MARK: Components
    private class func componentFlags() -> NSCalendarUnit { return [.Year, .Month, .Day,
        .WeekOfMonth, .Hour, .Minute, .Second, .WeekOfYear,
        .WeekdayOrdinal, .WeekOfYear] }
    ...
    func relativeTimeToString() -> String
    {
        ...
        if seconds < 10 {
            return NSLocalizedString("now", comment: "relative time")
        } else if seconds < 60 {
            return NSLocalizedString("\(Int(seconds)) s", comment: "relative time")
        }
        
        if minutes < 60 {
            if minutes == 1 {
                return NSLocalizedString("1 m", comment: "relative time")
            } else {
                return NSLocalizedString("\(Int(minutes)) m", comment: "relative time")
            }
        }
        
        if hours < 24 {
            if hours == 1 {
                return NSLocalizedString("1 h", comment: "relative time")
            } else {
                return NSLocalizedString("\(Int(hours)) h", comment: "relative time")
            }
        }
        
        if days < 7 {
            if days == 1 {
                return NSLocalizedString("1 d", comment: "relative time")
            } else {
                return NSLocalizedString("\(Int(days)) d", comment: "relative time")
            }
        }
